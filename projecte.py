...
def importar_hotels(fitxer,IFS):
    ...
    try:
        with open('fitxer', 'r', encoding='utf-8') as file:
            for linia in file[1:]:
                nom, codi_hotel, carrer, numero, codi_barri, codi_postal, telefon, latitutd, longitud, estrelles = linia[:-2].split(IFS)
                llista_hotels.append(Hotel(nom, codi_hotel, carrer, numero, codi_barri, codi_postal, telefon, latitutd, longitud, estrelles))
